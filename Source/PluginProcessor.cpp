    ...
    for (int i = 0; i < buffer.getNumSamples(); i++) {
        for (int channel = 0; channel < totalNumInputChannels; ++channel) {
            ...
            if (channel == 0) {
                ...
                float out = channelData[i] * (1 - mix/100) + out_hp * mix/100;
...
HarmonicExciterAudioProcessor::createParameters()
{
    ...
    param.push_back(std::make_unique<juce::AudioParameterFloat>("in", "In", -12.0f, 12.0f, 0.0f));
    param.push_back(std::make_unique<juce::AudioParameterFloat>("ax", "Ax", -12.0f, 12.0f, 0.0f));
    param.push_back(std::make_unique<juce::AudioParameterFloat>("out", "Out", -12.0f, 12.0f, 0.0f));
    param.push_back(std::make_unique<juce::AudioParameterFloat>("mix", "Mix", 0.0f, 100.0f, 0.0f));
